...
void draw() {
    ...
    int off_x = (cfg.width  - w) / 2;
    int off_y = (cfg.height - h) / 2;
    int x, y, k;
    for (x = 0; x < N; x ++) {
        for (y = 0; y < N; y ++) {
            for (k = 0; k < block_size; k ++) {
                ...
                io_write(AM_GPU_FBDRAW, off_x + x * s, off_y + y * s, color_buf, s, s, false);
        }
    }
...
void sw(){
    ...
    if(addr < 0x20000000 || addr > 0x20040000) {
        check(addr < (1 << ADDR_WIDTH), "Address out of range!");
        RAM[addr >> 2] = R[rs2];
    }
    else rs2_o = R[rs2];
    RAM_addr = addr;
...
int main(int argc, char **argv) {
    ...
    load_bin("../../logisim-bin/vga.bin", RAM);
    load_bin("../../logisim-bin/vga.bin", ROM);
